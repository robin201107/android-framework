...
public class StringHippoHttpRequest extends HippoHttpRequest<String> {
    ...
    public HippoResponse<String> parseResponse(NetworkResponse response) {
        ...
        if (response.isSuccess()) {
            try {
                hippoResponse = new HippoResponse<String>(new String(response.getData(), encoding));
            } catch (UnsupportedEncodingException e) {
                hippoResponse = new HippoResponse<String>(new HippoException("Oops! Unsupported encoding, you must kidding me!", e));
            }
        } else {
            hippoResponse = new HippoResponse<String>(new HippoException("Cannot parse response due to this error happen.", response.getError()));
        }
